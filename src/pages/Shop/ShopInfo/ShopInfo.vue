  ...
  <div class="shop-info" ref="infoContent">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item " v-for="(support,index) in info.supports"
               :key="index" :class="classType[support.type]">
      ...
      <section class="section">
        ...
        <div class="pic-wrapper" ref="ulDiv">
          <ul class="pic-list" ref="ulContent">
  ...
  import BScroll from 'better-scroll'
    export default {
      ...
      computed:{
        ...
        //从商品页面切换到当前页面时，info已经在商品页面时获取了
        mounted(){
          if(!this.info.pics){
            return
          }
          //(先点击的商品页面，此时info已经有数据)数据有了创建BScroll对象
          this._initScroll()
        },
       watch:{
          info(value) {
        ...
        methods:{
          _initScroll() {
            new BScroll(this.$refs.infoContent)
          //  动态计算ul的值
            const ul = this.$refs.ulContent
            const liWidth = 120
            const marginRight = 6
            const size = this.info.pics.length
            ul.style.width = (liWidth + marginRight) * size - marginRight + 'px'
          //   水平滑动
            new BScroll(this.$refs.ulDiv,{scrollX:true})
          },
        }
